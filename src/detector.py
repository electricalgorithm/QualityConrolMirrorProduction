...
def init_argument_parser():
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="Provide your image to calculate angles.",
    )
    parser.add_argument(
        "-t",
        "--tiles",
        nargs="+",
        required=True,
        help="Provide the tile count for dividing the image into sub-images.",
    )
    parser.add_argument(
        "-lf",
        "--log_file",
        nargs="+",
        required=True,
        help="The file location to save logs.",
    )
    parser.add_argument(
        "-sd",
        "--save_dir",
        nargs="+",
        required=True,
        help="The directory location to save step images.",
    )
...
if __name__ == "__main__":
    ...
    if len(args.tiles) != 2:
        raise RuntimeError(
            "You need to provide two integer values for tile structure.")
    ...
    angle_detector = AngleDetection(
        ObjectImage, (int(args.tiles[0]), int(args.tiles[1])), debug=debugger
    )
    angle_detector.run()
